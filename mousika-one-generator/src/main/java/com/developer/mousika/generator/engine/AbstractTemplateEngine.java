        ...
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                if (null != tableInfo.getDtoName() && null != pathInfo.get(ConstVal.DTO_PATH)) {
                    String dtoFile = String.format((pathInfo.get(ConstVal.DTO_PATH) + File.separator + tableInfo.getDtoName() + this.suffixJavaOrKt()), entityName);
                    if (isCreate(FileType.DTO, dtoFile)) {
                        writer(objectMap, templateFilePath(template.getDto()), dtoFile);
                    }
                }
                //MapStruct.java
                if (null != tableInfo.getMapStructName() && null != pathInfo.get(ConstVal.MAP_STRUCT_PATH)) {
                    String mapStructFile = String.format(pathInfo.get(ConstVal.MAP_STRUCT_PATH) + File.separator + tableInfo.getMapStructName() + this.suffixJavaOrKt(), entityName);
                    if (isCreate(FileType.MAPSTRUCT, mapStructFile)) {
                        writer(objectMap, templateFilePath(template.getMapStruct()), mapStructFile);
                    }
                }
                //Resource.java
                if (null != tableInfo.getMapStructName() && null != pathInfo.get(ConstVal.RESOURCE_PATH)) {
                    String resouceFile = String.format(pathInfo.get(ConstVal.RESOURCE_PATH) + File.separator + tableInfo.getResource() + this.suffixJavaOrKt(), entityName);
                    if (isCreate(FileType.RESOURCE, resouceFile)) {
                        writer(objectMap, templateFilePath(template.getResource()), resouceFile);
                    }
                }
                /**
                 * 自定义Angluar相关
                 * */
                //AngluarEntity.java
                if (null != tableInfo.getMapStructName() && null != pathInfo.get(ConstVal.ANGLUAR_ENTITY_PATH)) {
                    String angluarEntityFile = String.format(pathInfo.get(ConstVal.ANGLUAR_ENTITY_PATH) + File.separator + tableInfo.getAngluarEntity() + ".ts", entityName);
                    if (isCreate(FileType.ANGLUAR_ENTITY, angluarEntityFile)) {
                        writer(objectMap, templateFilePath(template.getAngluarEntity()), angluarEntityFile);
                    }
                }
                //AngluarHttpService.java
                if (null != tableInfo.getMapStructName() && null != pathInfo.get(ConstVal.ANGLUAR_HTTP_SERVICE_PATH)) {
                    String angluarHttpServiceFile = String.format(pathInfo.get(ConstVal.ANGLUAR_HTTP_SERVICE_PATH) + File.separator + tableInfo.getAngluarHttpService() + ".ts", entityName);
                    if (isCreate(FileType.ANGLUAR_HTTP_SERVICE, angluarHttpServiceFile)) {
                        writer(objectMap, templateFilePath(template.getAngluarHttpService()), angluarHttpServiceFile);
                    }
                }
            }
        } catch (Exception e) {
